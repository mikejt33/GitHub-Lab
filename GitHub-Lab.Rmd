...
4. A window will appear that asks you to clone the Git Repository. You can get the url from the big green button on your repository page on GitHub. Copy and paste that link there. This should populate the second field according to the repository's name but you can change it to what you'd like. You chan change where this is stored in the third field if you want.
...
When you created your project, a tab called `Git` was added to your `Environment` pane. In that tab, you can see the options `Diff`, `Commit`, `Pull`, `Push`, `History`, `More`, `New Branch` and, in my case, because I created the repository, `master`.
...
`pull`  is what you would do to bring a local branch up-to-date with its remote version, while also updating your other remote-tracking branches.
...
You can also edit your file directly on GitHub. This is not the best way to make big changes, but for small ones it is fine. This is text I added online.
